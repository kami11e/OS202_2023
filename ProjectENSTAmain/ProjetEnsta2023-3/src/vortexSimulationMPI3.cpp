...
int main( int nargs, char* argv[] )
{
    ...
    int* recvcountsCloud = new int[N_calcul];
    int* displsCloud = new int[N_calcul];
    for(int i=0;i<N_calcul;i++){
        recvcountsCloud[i] = subnbOfPts*2;
        displsCloud[i] = i*subnbOfPts0*2;
        ...
        while(signal>=0){
            if(signal>=2){
                ...
                if(signal){
                    ...
                    subCloud = sub_solve_RK4_vortices_new_cloud(dt, grid, cloud, rank, subnbOfPts0, subnbOfPts);
                    ...
                    subCloud = sub_solve_RK4_vortices_new_cloud(dt, grid, cloud, rank, subnbOfPts0, subnbOfPts0);
                    MPI_Allgather(subCloud.data(), subnbOfPts*2, MPI_DOUBLE, newCloudData.data(), subnbOfPts*2,  MPI_DOUBLE, scomm);
                    // MPI_Allgatherv(subCloud.data(), subnbOfPts*2, MPI_DOUBLE, newCloudData.data(), recvcountsCloud, displsCloud,  MPI_DOUBLE, scomm);
                    cloud = Geometry::CloudOfPoints (newCloudData);
                    if(rank==1){
