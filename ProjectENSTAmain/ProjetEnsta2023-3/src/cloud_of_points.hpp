...
namespace Geometry 
{
    ...
    public:
        ...
        {}
        CloudOfPoints( std::size_t nbPoints, double* m_setOfPoints_data )
            :   m_setOfPoints(nbPoints)
        {
            // m_setOfPoints = *((std::vector<point>*)m_setOfPoints_data);
        }
        CloudOfPoints( std::vector<double> const& m_coordinates ){
            unsigned long nbPoints = m_coordinates.size()/2;
            m_setOfPoints = container(nbPoints);
            for(unsigned long i=0; i<nbPoints;i++){
                m_setOfPoints[i] = point{m_coordinates[2*i], m_coordinates[2*i+1]};
            }
        }
