...
void 
CartesianGridOfSpeed::subUpdateVelocitySubField( Simulation::Vortices const& t_vortices, int rank, std::size_t subHeight)
{   
    int id=rank-1;
    using point=Simulation::Vortices::point;
    double halfStep = 0.5*m_step;
    for ( std::size_t idx=0, iRow=id*subHeight; idx<subHeight; ++iRow , ++idx)
    {
        double yP = m_bottom + iRow*m_step + halfStep;
        for ( std::size_t jCol=0; jCol<m_width; ++jCol)
        {
...
void CartesianGridOfSpeed::SetVelocityFieldVal( std::vector<double>const& t_grid){
    // std::cout<<"Update grid of speed, nbOfValues="<<t_grid.size()<<std::endl;
    ...
    for ( std::size_t iRow=0; iRow<m_height; ++iRow )
    {
        ...
        {
            std::size_t index = iRow*m_width + jCol;
            vector speed = {t_grid[2*index], t_grid[2*index+1]};
            // if( (index%80)==0) std::cout<<speed.x - m_velocityField[index].x<<"   "<<std::endl;
            // std::cout<<speed.x - m_velocityField[index].x<<"   "<<std::endl;
            m_velocityField[index].x = t_grid[2*index];
            m_velocityField[index].y = t_grid[2*index+1];
        }
    }
