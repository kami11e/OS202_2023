                                     ...
                                     Geometry::CloudOfPoints const& t_points, int id, unsigned long subnbOfPts0, unsigned long subnbOfPts )
{
    ...
    Geometry::CloudOfPoints newSubCloud(subnbOfPts);
    // std::cout<<"cloud.m_setOfPoints.size()=" <<t_points.numberOfPoints()<<std::endl;

    for ( std::size_t idx=0, iPoint=subnbOfPts0*(id-1);iPoint<t_points.numberOfPoints() &&  idx<subnbOfPts ; ++idx, ++iPoint)
    {
        // std::cout<<iPoint<<",   ";
        point  p = t_points[iPoint];
        vector v1 = t_velocity.computeVelocityFor(p);
...
// }
void
Numeric::solve_RK4_movable_vortices_update_vortices(double dt, CartesianGridOfSpeed& t_velocity, 
                                        Simulation::Vortices& t_vortices,
                                        Geometry::CloudOfPoints const& t_points)
{
    using vector = Simulation::Vortices::vector;
