...
int main( int nargs, char* argv[] )
{
    ...
    if(rank != P_AFFICHAGE){
        ...
        // MPI_Bcast(&signal, 1, MPI_INT, P_CALCUL, 200, MPI_COMM_WORLD);
        ...
        while(signal>=0){
            if(signal>=2){
                ...
                std::vector<Geometry::Point<double>> subCloud;
                unsigned long nbOfVts=vortices.numberOfVortices(), nbOfPts, subnbOfPts;
                std::vector<double> newCloudData(nbOfPts*2);
                if(signal){
                    // std::cout<<"--------"<<std::endl;
                    cloud = Numeric::solve_RK4_movable_vortices(dt, grid, vortices, cloud);
                    nbOfPts= cloud.numberOfPoints();
                    // std::cout<<"Process["<<rank<<"] send Info Of Pts="<<nbOfPts<<" to Process["<<P_AFFICHAGE<<"]"<<std::endl;
                    MPI_Send(&nbOfPts, 1, MPI_UNSIGNED_LONG, P_AFFICHAGE, 399, MPI_COMM_WORLD);
                    MPI_Send(cloud.data(), nbOfPts* 2, MPI_DOUBLE, P_AFFICHAGE, 400, MPI_COMM_WORLD);
                    // std::cout<<"Process["<<rank<<"] send Cloud Data to Process["<<P_AFFICHAGE<<"]"<<std::endl;
                    MPI_Send(vortices.data(), nbOfVts*3, MPI_DOUBLE, P_AFFICHAGE, 401, MPI_COMM_WORLD);
                    // std::cout<<"Process["<<rank<<"] send Vortice Data to Process["<<P_AFFICHAGE<<"]"<<std::endl;
                    MPI_Send(grid.data(), nbOfGridData*2, MPI_DOUBLE, P_AFFICHAGE, 402, MPI_COMM_WORLD);
                    // std::cout<<"Process["<<rank<<"] send Grid Data to Process["<<P_AFFICHAGE<<"]"<<std::endl;
        ...
        while (myScreen.isOpen())
        {
            ...
            while (myScreen.pollEvent(event))
            {
                ...
                if (event.type == sf::Event::Closed){
                    MPI_Send(&Close, 1, MPI_INT, P_CALCUL, 200, MPI_COMM_WORLD);
                    myScreen.close();
                }
                ...
                if (sf::Keyboard::isKeyPressed(sf::Keyboard::Up)) {
                    dt *= 2;
                    // std::cout<<"Process["<<rank<<"] send sig(Up)="<<Up<<"to Process["<<P_CALCUL<<"]"<<std::endl;
                    MPI_Send(&Up, 1, MPI_INT, P_CALCUL, 200, MPI_COMM_WORLD);
                    // MPI_Bcast(&Up, 1, MPI_INT, P_CALCUL, 200, MPI_COMM_WORLD);
                }
                
                if (sf::Keyboard::isKeyPressed(sf::Keyboard::Down)) {                    
                    // std::cout<<"Process["<<rank<<"] send sig(Down)="<<Down<<"to Process["<<P_CALCUL<<"]"<<std::endl;
                    dt /= 2;
                    MPI_Send(&Down, 1, MPI_INT, P_CALCUL, 200, MPI_COMM_WORLD);
                    // MPI_Bcast(&Down, 1, MPI_INT, P_CALCUL, 200, MPI_COMM_WORLD);
                }
            ...
            if (animate | advance)
            {
                ...
                if (isMobile)
                {
                    ...
                    // std::cout<<"Process["<<rank<<"] send sig(isMobile)="<<isMobile<<"to Process["<<P_CALCUL<<"]"<<std::endl;
                    MPI_Send(&isMobile, 1, MPI_INT, P_CALCUL, 200, MPI_COMM_WORLD);
                    MPI_Recv(&nbOfPts,  2, MPI_UNSIGNED_LONG, P_CALCUL, 399, MPI_COMM_WORLD, &status);

                    // std::cout<<"Process["<<rank<<"] recv nb Of Pts="<<nbOfPts<<" from Process["<<P_CALCUL<<"]"<<std::endl;
                    // std::cout<<"Process["<<rank<<"] recv nb Of Vortices="<<recvInfo[0]<<" from Process["<<P_CALCUL<<"]"<<std::endl;
                    std::vector<double> newCloudData(nbOfPts*2);
                    // std::cout<<"Process["<<rank<<"] prepare recv Cloud Data from Process["<<P_CALCUL<<"]"<<std::endl;
                    MPI_Recv(newCloudData.data(), nbOfPts*2, MPI_DOUBLE, P_CALCUL, 400, MPI_COMM_WORLD, &status);
                    // std::cout<<"Process["<<rank<<"] recv Cloud Data from Process["<<P_CALCUL<<"]"<<std::endl;
                    std::vector<double> newVorticeData(nbOfVts* 3);
                    MPI_Recv(newVorticeData.data(), nbOfVts*3, MPI_DOUBLE, P_CALCUL, 401, MPI_COMM_WORLD, &status);
                    // std::cout<<"Process["<<rank<<"] recv Vortice Data from Process["<<P_CALCUL<<"]"<<std::endl;
                    ...
                    MPI_Recv(newGridData.data(), nbOfGridData*2, MPI_DOUBLE, P_CALCUL, 402, MPI_COMM_WORLD, &status);
                ...
                {
                            // no MPI parallized
                    unsigned long nbOfPts;
                    // cloud = Numeric::solve_RK4_fixed_vortices(dt, grid, cloud);
                    // std::cout<<"Process["<<rank<<"] send sig(isMobile)="<<isMobile<<"to Process["<<P_CALCUL<<"]"<<std::endl;
                    MPI_Send(&isMobile, 1, MPI_INT, P_CALCUL, 200, MPI_COMM_WORLD);
                    MPI_Recv(&nbOfPts,  1, MPI_UNSIGNED_LONG, P_CALCUL, 399, MPI_COMM_WORLD, &status);
                    // std::cout<<"Process["<<rank<<"] recv nb Of Pts="<<nbOfPts<<" from Process["<<P_CALCUL<<"]"<<std::endl;
                    std::vector<double> newCloudData(nbOfPts*2);
                    // std::cout<<"Process["<<rank<<"] prepare recv Cloud Data from Process["<<P_CALCUL<<"]"<<std::endl;
                    MPI_Recv(newCloudData.data(), nbOfPts*2, MPI_DOUBLE, P_CALCUL, 400, MPI_COMM_WORLD, &status);
